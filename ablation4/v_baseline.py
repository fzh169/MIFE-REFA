...
class MyModel(nn.Module):
    def __init__(self, args):
        ...
        self.flow1 = BasicFlow(dim=128, part_size=8, up_factor=4)
        self.flow2 = BasicFlow(dim=192, part_size=16, up_factor=8)
        self.flow3 = BasicFlow(dim=256, part_size=16, up_factor=16)
    ...
    def forward(self, frame0, frame2):
        ...
        # frames0 = F.interpolate(frame0, scale_factor=0.5, mode="bilinear")
        # frames2 = F.interpolate(frame2, scale_factor=0.5, mode="bilinear")

        feat00, feat10, feat20, feat30 = self.fe(frame0 - mean_f)
        feat02, feat12, feat22, feat32 = self.fe(frame2 - mean_f)

        flow_0, range_0, corr02_0, corr20_0 = self.flow0(feat00, feat02)
        flow_1, range_1, corr02_1, corr20_1 = self.flow1(feat10, feat12)
        flow_2, range_2, corr02_2, corr20_2 = self.flow2(feat20, feat22)
        flow_3, range_3, corr02_3, corr20_3 = self.flow3(feat30, feat32)
...
class BasicFlow(nn.Module):
    ...
    def forward(self, feat0, feat2):
        ...
        q0, q2 = self.proj_q0(feat0), self.proj_q0(feat2)
        k0, k2 = self.proj_k0(feat0), self.proj_k0(feat2)
        ...
        flow12, corr12, range1, flow02, corr02, range0 = self.estimate(q0, k2, pos, bias, s1, s2, H, W)
        flow10, corr10, range1, flow20, corr20, range0 = self.estimate(q2, k0, pos, bias, s1, s2, H, W)
        ...
        return (flow10, flow12, flow02, flow20), (range1, range1, range0, range0), corr02, corr20
