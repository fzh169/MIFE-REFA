...
class MyModel(nn.Module):
    def __init__(self, args):
        ...
        self.fe = FeatureExtractor(dim=(32, 64, 128))
    ...
    def forward(self, frame0, frame2):
        ...
        feat10 = self.fe(frame0 - mean_f)
        feat12 = self.fe(frame2 - mean_f)

        flow10_1, flow12_1, flow02_1, flow20_1 = self.flow1(feat10, feat12)

        flow10 = self.fn(flow10_1)
        flow12 = self.fn(flow12_1)
        flow02 = self.fn(flow02_1)
        flow20 = self.fn(flow20_1)
        ...
        if self.training:
            ...
            return {'frame1': frame1_hat, 'g1': g1}
        else:
